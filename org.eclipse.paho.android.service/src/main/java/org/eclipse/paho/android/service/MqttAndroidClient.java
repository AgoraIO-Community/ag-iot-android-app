...
public class MqttAndroidClient extends BroadcastReceiver implements
		IMqttAsyncClient {
	private final String TAG = "IOTSDK/MqttAClient";
	/**
	 * 
	...
	public void setMqttLogger(final MqttLog.IMqttLogger logger) {
		MqttLog.setMqttLogger(logger);
	}
...
//			}
	...
	public void setTraceCallback(MqttTraceHandler traceCallback) {
		MqttLog.d(TAG, "<setTraceCallback>");
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (MqttServiceConstants.CONNECT_ACTION.equals(action)) {
			...
			MqttLog.d(TAG, "<onReceive> SUBSCRIBE_ACTION");
			subscribeAction(data);
		}
		else if (MqttServiceConstants.UNSUBSCRIBE_ACTION.equals(action)) {
			MqttLog.d(TAG, "<onReceive> UNSUBSCRIBE_ACTION");
			...
			messageDeliveredAction(data);
				...
				.equals(action)) {
			MqttLog.d(TAG, "<onReceive> ON_CONNECTION_LOST_ACTION");
			connectionLostAction(data);
		}
		else if (MqttServiceConstants.DISCONNECT_ACTION.equals(action)) {
			MqttLog.d(TAG, "<onReceive> DISCONNECT_ACTION");
			disconnected(data);
		}
	...
	private void disconnected(Bundle data) {
		MqttLog.d(TAG, "<disconnected> data=" + data);
		// TODO: @2023-07-26 luxiaohua: 要到关闭的时候再清除
		//clientHandle = null; // avoid reuse!
	...
	private void connectionLostAction(Bundle data) {
		if (callback != null) {
			...
			MqttLog.d(TAG, "<connectionLostAction> reason=" + reason);

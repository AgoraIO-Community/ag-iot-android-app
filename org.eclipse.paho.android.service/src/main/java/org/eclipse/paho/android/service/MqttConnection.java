...
class MqttConnection implements MqttCallbackExtended {
	...
	private static final String TAG = "IOTSDK/MqttConnection";
	...
	void disconnect(String invocationContext, String activityToken) {
		MqttLog.d(TAG, "<disconnect> invocationContext=" + invocationContext
		...
		resultBundle.putString(MqttServiceConstants.CALLBACK_ACTION,
				MqttServiceConstants.DISCONNECT_ACTION);
		if ((myClient != null) && (myClient.isConnected())) {
			MqttLog.d(TAG, "<disconnect> mClient.disconnect()");
			IMqttActionListener listener = new MqttConnectionListener(
					resultBundle);
			try {
			...
			MqttLog.d(TAG, "<disconnect> callbackToActivity()");
			resultBundle.putString(MqttServiceConstants.CALLBACK_ERROR_MESSAGE,
					NOT_CONNECTED);
			service.traceError(MqttServiceConstants.DISCONNECT_ACTION,
					NOT_CONNECTED);
	...
	public boolean isConnected() {
		...
		if (myClient == null) {
			MqttLog.d(TAG, "<isConnected> myClient is NULL");
			return false;
		}

		boolean connected = myClient.isConnected();
		MqttLog.d(TAG, "<isConnected> connected=" + connected);
		return connected;
	}
	...
	public void subscribe(final String topic, final int qos,
		...
		MqttLog.d(TAG, "<subscribe> topic=" + topic + ", qos=" + qos);
		final Bundle resultBundle = new Bundle();
		resultBundle.putString(MqttServiceConstants.CALLBACK_ACTION,
	...
	public void subscribe(final String[] topic, final int[] qos,
		...
		service.traceDebug(TAG, "subscribe({" + Arrays.toString(topic) + "}," + Arrays.toString(qos) + ",{"
				+ invocationContext + "}, {" + activityToken + "}");
		MqttLog.d(TAG, "<subscribe> array, topic=" + topic + ", qos=" + qos);
		final Bundle resultBundle = new Bundle();
		resultBundle.putString(MqttServiceConstants.CALLBACK_ACTION,
				MqttServiceConstants.SUBSCRIBE_ACTION);
		resultBundle.putString(MqttServiceConstants.CALLBACK_ACTIVITY_TOKEN,
		...
		if ((myClient != null) && (myClient.isConnected())) {
			...
			try {
				...
				handleException(resultBundle, e);
				MqttLog.d(TAG, "<subscribe> array, exception, exp=" + e);
			}
		} else {
			resultBundle.putString(MqttServiceConstants.CALLBACK_ERROR_MESSAGE,
					NOT_CONNECTED);
			service.traceError("subscribe", NOT_CONNECTED);
			MqttLog.d(TAG, "<subscribe> array, NOT_CONNECTED");
			service.callbackToActivity(clientHandle, Status.ERROR, resultBundle);
		}
	}
	...
	public void connectionLost(Throwable why) {
		...
		try {
			if(!this.connectOptions.isAutomaticReconnect()) {
				...
				MqttLog.d(TAG, "<connectionLost> schedule 100ms for ping sender!");
		...
		MqttLog.d(TAG, "<connectionLost> callbackToActivity(ON_CONNECTION_LOST_ACTION)!");
		Bundle resultBundle = new Bundle();
		resultBundle.putString(MqttServiceConstants.CALLBACK_ACTION,
	...
	*/
	synchronized void reconnect() {
		...
		if (myClient == null) {
			...
			MqttLog.d(TAG, "<reconnect> myClient is NULL!");
			return;
		}

		if (isConnecting) {
			service.traceDebug(TAG, "The client is connecting. Reconnect return directly.");
			MqttLog.d(TAG, "<reconnect> myClient is connecting!");
			return ;
		}
		...
		if(!service.isOnline()){
			...
			MqttLog.d(TAG, "<reconnect> service NOT online!");
			return;
		}
		...
		if(connectOptions.isAutomaticReconnect()){
			...
			MqttLog.d(TAG, "<reconnect> Requesting Automatic reconnect using New Java AC");
			final Bundle resultBundle = new Bundle();
			resultBundle.putString(
					MqttServiceConstants.CALLBACK_ACTIVITY_TOKEN,
			...
			try {
				...
				MqttLog.e(TAG, "<reconnect> [MQTT_EXCEPTION] exp=" + ex);
				setConnectingState(false);
			...
			MqttLog.d(TAG, "<reconnect> Do Real Reconnecting...");
			final Bundle resultBundle = new Bundle();
			resultBundle.putString(
				MqttServiceConstants.CALLBACK_ACTIVITY_TOKEN,
				reconnectActivityToken);
			...
			try {
				...
				IMqttActionListener listener = new MqttConnectionListener(resultBundle) {
					...
					public void onSuccess(IMqttToken asyncActionToken) {
						...
						MqttLog.d(TAG, "<reconnect.onSuccess> Reconnect Success!");
						doAfterConnectSuccess(resultBundle);
					}
					
					@Override
					public void onFailure(IMqttToken asyncActionToken, Throwable exception) {
						MqttLog.d(TAG, "<reconnect.onFailure> Reconnect failure!");
						resultBundle.putString(
								MqttServiceConstants.CALLBACK_ERROR_MESSAGE,
								exception.getLocalizedMessage());
						resultBundle.putSerializable(
				...
				MqttLog.e(TAG, "<reconnect> [MQTT_EXCEPTION] exp=" + e);
				setConnectingState(false);
				handleException(resultBundle, e);
			} catch (Exception e){
				...
				*/
				service.traceError(TAG, "Cannot reconnect to remote server." + e.getMessage());
				MqttLog.e(TAG, "<reconnect> [EXCEPTION] exp=" + e);
				setConnectingState(false);

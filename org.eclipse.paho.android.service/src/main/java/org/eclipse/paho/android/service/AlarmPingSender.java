...
class AlarmPingSender implements MqttPingSender {
	...
	private static final String TAG = "IOTSDK/AlarmPingSender";
	...
	public void schedule(long delayInMilliseconds) {
		long nextAlarmInMilliseconds = System.currentTimeMillis()
				+ delayInMilliseconds;
		MqttLog.d(TAG, "<schedule> delayInMilliseconds=" + delayInMilliseconds
					+ ", nextAlarmInMilliseconds=" + nextAlarmInMilliseconds);
        ...
        if(Build.VERSION.SDK_INT >= 23){
			...
			MqttLog.d(TAG, "<schedule> setExactAndAllowWhileIdle()");
	...
	class AlarmReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			MqttLog.d(TAG, "<onReceive> sending ping packet!");
			...
			IMqttToken token = comms.checkForActivity(new IMqttActionListener() {
				...
				public void onSuccess(IMqttToken asyncActionToken) {
					...
					MqttLog.d(TAG, "<onReceive.onSuccess> ");
				...
				public void onFailure(IMqttToken asyncActionToken,
					...
					Log.d(TAG, "Failure. Release lock(" + wakeLockTag + "):"
							+ System.currentTimeMillis());
					MqttLog.d(TAG, "<onReceive.onFailure> exception=" + exception);
					//Release wakelock when it is done.
					wakelock.release();
				}
			});

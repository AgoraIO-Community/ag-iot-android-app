...
public class MqttService extends Service implements MqttTraceHandler {
	...
	static final String TAG = "IOTSDK/MqttService";
  ...
  private Map<String/* clientHandle */, MqttConnection/* client */> mOpenedConns = new ConcurrentHashMap<>();
  ...
  public String getClient(String serverURI, String clientId, String contextId, MqttClientPersistence persistence) {
    ...
    if (!connections.containsKey(clientHandle)) {
      ...
      // TODO: @2023-07-26 luxiaohua: 添加到打开链路的映射表中
      if (!mOpenedConns.containsKey(clientHandle)) {
        mOpenedConns.put(clientHandle, client);
      }
    }
  ...
  void reconnect()  {
	traceDebug(TAG, "Reconnect to server, client size=" + connections.size());
      MqttLog.d(TAG, "<reconnect.onReceive> clientSize=" + connections.size());
  ...
  public void close(String clientHandle) {
    MqttLog.d(TAG, "<close> clientHandle=" + clientHandle);

    // TODO: @2023-07-26 luxiaohua: 这里不能再从 connections 获取链路了，需要从 mOpenedConns中获取
    // MqttConnection client = getConnection(clientHandle);
    MqttConnection client = mOpenedConns.get(clientHandle);
    client.close();
    mOpenedConns.remove(clientHandle);
  ...
  public void disconnect(String clientHandle, String invocationContext,
    ...
    MqttLog.d(TAG, "<disconnect> clientHandle=" + clientHandle
            + ", invocationContext=" + invocationContext + ", activityToken=" + activityToken);
    MqttConnection client = getConnection(clientHandle);
    client.disconnect(invocationContext, activityToken);
    connections.remove(clientHandle);  // TODO: @2023-07-26 luxiaohua: close()的时候不能通过connections获取链路了
    ...
    stopSelf(-1);  // TODO: @2023-07-26 luxiaohua: 当前只有1个链路，直接关闭所有服务
  }
  ...
  public void disconnect(String clientHandle, long quiesceTimeout,
      String invocationContext, String activityToken) {
    MqttLog.d(TAG, "<disconnect> clientHandle=" + clientHandle + ", quiesceTimeout=" + quiesceTimeout
            + ", invocationContext=" + invocationContext + ", activityToken=" + activityToken);
    MqttConnection client = getConnection(clientHandle);
  ...
  public void onCreate() {
	...
	public void onDestroy() {
        MqttLog.d(TAG, "<onDestroy> BEGIN");

		// disconnect immediately
		for (MqttConnection client : connections.values()) {
            MqttLog.d(TAG, "<onDestroy> disconnect client: " + client.getClientId());
  ...
  public IBinder onBind(Intent intent) {
    ...
    MqttLog.d(TAG, "<onBind> activityToken=" + activityToken);
  ...
  public int onStartCommand(final Intent intent, int flags, final int startId) {
    ...
    MqttLog.d(TAG, "<onStartCommand> startId=" + startId);
    return START_STICKY;
  }
  ...
  private class NetworkConnectionIntentReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
            ...
            MqttLog.d(TAG, "<NetworkConnectionIntentReceiver.onReceive> Internal network status receive");
			...
			if (isOnline()) {
                ...
                MqttLog.d(TAG, "<NetworkConnectionIntentReceiver.onReceive> online, reconnecting...");
				// we have an internet connection - have another try at
				// connecting
				reconnect();
			} else {
                MqttLog.d(TAG, "<NetworkConnectionIntentReceiver.onReceive> notify off line");
	...
	private class BackgroundDataPreferenceReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
            ...
            boolean bgDataSetting = cm.getBackgroundDataSetting();
            MqttLog.d(TAG, "<BackgroundDataPreferenceReceiver.onReceive> bgDataSetting=" + bgDataSetting);
